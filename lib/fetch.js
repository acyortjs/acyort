...
module.exports = function(callback) {
    ...
    function opt(page) {
        ...
        if (config.dev) {
            ...
            path = config.dev_path +'issues'+ page +'.json'
        } else {
            host = 'api.github.com';
            path = '/repos/'+ config.issue_repo +'/issues?page='+ page +'&per_page=20'+ (config.token ? '&access_token='+ config.token.split(' ').join('') : '')
        }
    ...
    function response(res) {
        ...
        res.on('end', function() {
            console.log('fetch data ['+ page +']...Done')
