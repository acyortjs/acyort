...
class Acyort {
  constructor(config) {
    ...
    this.extend = new Extend(this, extends)
    this.builder = new Builder(config, this.helper)
    this.fetchData = null
    this.processData = null
  }

  start(port) {
    function trigger({ path, clients }) => {
    ...
    this.server.addTrigger(trigger)
  ...
  build(type) {
    ...
    return this.extend.init()
      .then(() => {
        ...
        Object.keys(helpers).forEach(h => this.helper.addMthod(h, helpers[h]))
