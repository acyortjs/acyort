...
class Render extends Renderer {
  ...
  _data(page) {
    return Object.assign({ page }, this.helper.functions)
  }
  ...
  genHtmls(data, ...types) {
    ...
    if (args.indexOf('categories') > -1) {
      this.render(this._path('category'), 'categories', this._data(categories))
    }

    if (args.indexOf('tags') > -1) {
      this.render(this._path('tag'), 'tags', this._data(tags))
    }
