...
class Processor {
  constructor(config) {
    this._paginationFn = data => new Pagination(config).get(data)
    this._jsonifyFn = data => new Jsonify(config).get(data)
    this._config = config
    this.categories = []
    this.tags = []
    this.posts = []
  ...
  _setPaginations() {
    ...
    this.paginations.page = this._paginationFn({ name: title, posts })
    this.paginations.archives = this._paginationFn({ name: title, type: 'archives', posts })
    this.categories.forEach((c) => {
      const category = Object.assign({ type: 'category' }, _.pick(c, ['name', 'id', 'posts']))
      this.paginations.categories[c.id] = this._paginationFn(category)
    })
    this.tags.forEach((t) => {
      const tag = Object.assign({ type: 'tag' }, _.pick(t, ['name', 'id', 'posts']))
      this.paginations.tags[t.id] = this._paginationFn(tag)
    })
  }
  ...
  _post(post) {
    ...
    const data = {
      ...
      toc: postToc(body),
      category: postCategory(post, this._config),
      tags: postTag(post, this._config),
    }

    const { thumb, content } = postThumb(body, this._config)
  ...
  process(issues) {
    const { pages, posts } = issueFilter(issues, this._config)
    ...
    return Promise.resolve(json ? this._jsonifyFn(data) : data)
  }
}
