...
module.exports = function(data) {
    ...
    authors_data.forEach(function(e, i) {
        ...
        // thumb
        var reg = /(!\[.*?\]\()(.+?)(\))/;
        reg = e.body.match(reg);
        if (reg) {
            //console.log(reg[2].split('"')[0].trim())
        }
        ...
        if (i == 0) {
            var t = moment(e.created_at).format('M, MM, MMM, MMMM, D, Do, DD, ddd, dddd, YYYY');
            console.log(t)
        }
    ...
    return {
        ...
        categories: util.unique(categories)
    }
