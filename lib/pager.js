...
module.exports = function(data, type) {
    ...
    if (data.length > config.per_page) {
        ...
        for (var i = 0; i < data.length; i += config.per_page) {
            ...
            pager.next = {css: '', url: config.url +'/'+ path +'/'+ (page + 1) +'/'}
            if (page != 1) {
                pager.prev = {css: '', url: config.url +'/'+ path +'/'+ (page - 1) +'/'}
            }
            if (page == 2) {
                pager.prev = {css: '', url: type == 'index' ? config.url +'/' : config.url +'/'+ path +'/'}
            }
