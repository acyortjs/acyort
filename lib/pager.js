...
module.exports = function(data, type, templates, all) {
    ...
    var pager = {
        ...
        prev: '',
        next: '',
        heading: type.split('/')[1] || config.title,
        tags: all.tags.map(function(tag) {
            return {
                name: tag.name,
                url: tag.url,
                number: tag.number,
                class: type.split('/')[1].replace(/\s/g, '_') +' '+ (type.split('/')[1] == tag.name ? 'current_tag' : '')
            }
        }),
        categories: all.categories.map(function(category) {
            return {
                ...
                class: type.split('/')[1].replace(/\s/g, '_') +' '+ (type.split('/')[1] == category.name ? 'current_category' : '')
            }
        })
    }
