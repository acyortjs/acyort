...
module.exports = function(issues) {
    ...
    paginations.archives = _paginations(pid, 'archives')
    categories.forEach(category => {
        paginations.categories[category.name] = _paginations(category.posts, _path.join(config.category_dir.transform(), category.name))
    })
    tags.forEach(tag => {
        paginations.tags[tag.name] = _paginations(tag.posts, _path.join(config.tag_dir.transform(), tag.name))
    })
