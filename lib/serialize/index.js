...
module.exports = function(issues) {
    ...
    categories.forEach(category => {
        paginations.categories[category.name.transform()] = _paginations(category.posts, _path.join(config.category_dir.transform(), category.name))
    })
    tags.forEach(tag => {
        paginations.tags[tag.name.transform()] = _paginations(tag.posts, _path.join(config.tag_dir.transform(), tag.name))
    })

    return Promise.resolve(paginations)
