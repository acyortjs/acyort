...
module.exports = function(data) {
    ...
    data.forEach(function(e, i) {
        ...
        if (e.title.indexOf('[') > -1 && e.title.indexOf(']') > -1) {
            ...
            posts[i].category_name = posts[i].categories;

            // [{name: category name, url: category url}, {}...]
            posts[i].categories = posts[i].categories.map(function(category) {
                return {
                    name: category,
            ...
            // [tag name, tag name, ...]
            posts[i].tag_name = posts[i].tags;

            // [{name: tag name, url: tag url}, {}...]
            posts[i].tags = posts[i].tags.map(function(tag) {
                return {
                    name: tag,
    ...
    tags.forEach(function(tag) {
        tag.posts = posts.filter(function(post) {
            return post.tag_name.indexOf(tag.name) > -1
        })
    ...
    categories.forEach(function(category) {
        category.posts = posts.filter(function(post) {
            return post.category_name.indexOf(category.name) > -1
        })
